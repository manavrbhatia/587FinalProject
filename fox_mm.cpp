...
int main(int argc, char**argv) { 
    ...
    double* A_buffer = (double*) calloc(N*N*sizeof(double));
    ...
    for(int i = 0; i < p_rt; i++){
        ...
        if (p_j == (p_i+i) % p_rt){
            for (int b_idx = 0; b_idx < p_rt; i++) {
                // Broadcast to everyone using non blocking send
                if (p_j != (p_j+b_idx+1)%p_rt) 
                    MPI_Isend(A_sub, N*N, MPI_DOUBLE, idx(p_i,(p_j+b_idx+1)%p_rt,p_rt), 0, MPI_COMM_WORLD);
            }
        }
        else
            MPI_Recv(A_buffer, N*N, MPI_DOUBLE, idx(p_i,(p_i+1)%p_rt,p_rt), 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            for (int b=0;b<N;b++) {
                for (int c=0;c<N;c++) {
                    if (p_j == (p_i+i) % p_rt)
                        ...
                        C_sub[idx(a,b,N)] += A_buffer[idx(a,c,N)]*B_sub[idx(c,b,N)];
                }
            }
        }
        MPI_Barrier(MPI_COMM_WORLD);
        
        // Send B submatrices upwards, confirm this is correct
